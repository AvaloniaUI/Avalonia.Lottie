...
namespace Avalonia.Lottie.Parser
{
    internal static class JsonUtils
    {
        ...
        internal static List<Vector2> JsonToPoints(JsonReader reader, double  scale)
        {
        ...
        internal static Vector2 JsonToPoint(JsonReader reader, double  scale)
        {
            switch (reader.Peek())
            {
        ...
        private static Vector2 JsonNumbersToPoint(JsonReader reader, double  scale)
        {
            var x = reader.NextDouble();
            var y = reader.NextDouble();
            while (reader.HasNext()) reader.SkipValue();
            return new Vector2((float)x * (float)scale, (float)y * (float)scale);
        }

        private static Vector2 JsonArrayToPoint(JsonReader reader, double  scale)
        {
            double  x;
            double  y;
            reader.BeginArray();
        ...
        private static Vector2 JsonObjectToPoint(JsonReader reader, double  scale)
        {
            var x = 0d;
            var y = 0d;
            reader.BeginObject();
